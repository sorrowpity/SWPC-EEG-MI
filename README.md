...
## 使用说明：
在真实场景下，推荐使用CSP&FBCSP来判断当前是否处于静息态或运动想象态，EEGNet的性能或许更高，但是不稳定。而在明确当前为运动想象态的前提下，推荐使用正文中所说 EEGNET先supervised-learning再接self-supervised learning来分类，会取得更好的效果。
...
### 2026-02-13
另加了train_swpc_SSL.py和train_swpc_Lyapunov.py及其配套的ShallowConvNet文件，
SSL表示加了自监督，但是我发现加了自监督反而效果变差
Lyapunov表示脑电信号的‘混乱度’指标，在第一步预筛选时，不仅让模型学 “是不是 MI”，还让它学 “信号混不混乱”（用 LE 量化），相当于给模型多了一个 “判断依据”，帮它更精准地区分静息态和 MI 态。

最终加入了‘混乱度’指标后，让模型测试结果整体提高了5%

### 2026-03-19
修改了train_swpc_4ch_4cls_BCI2a.py的训练代码，修改了stage1的自监督和监督训练顺序为先监督后自监督
加入了stage1和stage2的模型级联