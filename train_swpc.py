...
# 配置参数
BATCH_SIZE = 32
EPOCHS = 50  # 演示用，实际建议 100+
LR = 0.0001
WEIGHT_DECAY = 1e-4 # 设置一个较强的L2正则化，常用值在1e-4到1e-2
DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
TEST_SUBJ = 1  # 假设用被试1做测试，其他做训练
...
def main():
    ...
    # 4. 初始化模型 (2分类)
    # 注意：根据你的loader输出调整 channels 和 time_points
    channels = X_train.shape[1]
    time_points = X_train.shape[2]
    model_prescreen = ShallowConvNet(num_classes=2, channels=channels, time_points=time_points, dropout_rate=0.5).to(DEVICE)
